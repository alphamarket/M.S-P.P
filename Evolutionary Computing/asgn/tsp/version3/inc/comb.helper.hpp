...
vector<size_t> get_cross_points(const size_t no, const size_t lower_bound, const size_t upper_bound, bool use_blind_approach = false) {
    vector<size_t> p;
    do {
        p.clear();
        size_t size = no;
        ...
        // should perform 1-time loop?
        if(use_blind_approach) break;
